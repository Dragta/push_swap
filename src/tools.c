...
int	mean(t_compendium *all, int stk)
{
	...
	if ((m == all->top[stk]->target && all->max[stk] > 2) ||
		sense(all->top[stk]->target, m, max) != 1 - 2 * stk)
		return ((m + max / 2) % max);
	return (m);
}
...
int	closest(t_compendium *all, t_data *i, int toler)
{
	...
	tmp = i->prev;
	ct_down = 0;
	while (++ct_down < all->max[i->id] / 2 && !where_to_push(all, tmp, toler))
		tmp = tmp->prev;
	tmp = i;
	ct_up = -1;
	while (++ct_up <= all->max[i->id] / 2 && !where_to_push(all, tmp, toler))
		tmp = tmp->next;
	if (ct_up + ct_down >= all->max[i->id])
		return (-1)
	if (ct_up > ct_down)
		return (3);
	return (0);
...
int	ordered(t_compendium *all)
{
	int	stk;
	t_data	*i;

	if (all->top[1] && all->top[0])
		return (0);
	stk = 0;
	if (all->top[1])
		stk = 1;
...
/*	show_all(all);*/
	...
	stk = 0;
	pass = 1;
	toler = 7 * all->max_val / 100 + 3;
/*	stay[0] = (*top_[stk])->target;*/
	while (pass < 3 && toler)
	{
/*		printf("pass %i\n", pass);
		...
		if (ordered(all))
		{
			while ((*top_[stk])->target > all->max_val / 2)
				move(all, _RA + stk);
			while ((*top_[stk])->target > 0)
				move(all, _RRA + stk);
			break;
		}
		init_st = all->n_st;
		...
		if (!all->max[1 - stk])
			move(all, _PB - stk);
		while (all->n_st < 13000 && all->max[stk] > 3)/* && ct <= 5 * all->max[stk]) && init_target != (*top_[stk])->target)*/
		{
			...
			if (way < 0)
			{
				while (all->n_st < 13000 && abs((*top_[stk])->target - (*top_[1 - stk])->target) > toler)
					move(all, _RRB - stk);
			}
			way = closest(all, *top_[stk], toler);
			if (way < 0)
				break ;*/
			init_st = all->n_st;
			while (!where_to_push(all, *top_[stk], toler) && all->n_st - init_st < all->max[stk])
				move(all, _RA + stk + way);
			if (all->n_st - init_st >= all->max[stk])
				break ;
